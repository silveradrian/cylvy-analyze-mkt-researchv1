...
async def main() -> None:
    ...
    write_csv(os.path.join(exports_dir, f"organic_dsi_{pipeline_id[:8]}.csv"), organic)
    write_csv(os.path.join(exports_dir, f"news_dsi_{pipeline_id[:8]}.csv"), news)
    write_csv(os.path.join(exports_dir, f"youtube_dsi_{pipeline_id[:8]}.csv"), youtube)
    ...
    write_csv(os.path.join(exports_dir, f"dsi_scores_{pipeline_id[:8]}.csv"), [dict(r) for r in rows])
