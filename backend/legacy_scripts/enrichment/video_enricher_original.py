...
class VideoEnricher:
    ...
    async def _fetch_videos_from_api(self, video_ids: List[str]) -> List[YouTubeVideoStats]:
        ...
        for i in range(0, len(video_ids), 50):
            ...
            async with self._semaphore:
                try:
                    # Execute directly without threading - Google API client is not thread-safe
    ...
    async def _enrich_with_channel_data(self, snapshots: List[VideoSnapshot]):
        ...
        # Extract company domains from channels and classify ownership
        await self._extract_channel_company_domains(snapshots)
    ...
    async def _get_cached_video(self, video_id: str) -> Optional[YouTubeVideoStats]:
        ...
        if self.redis:
            try:
                ...
                # Redis is thread-safe, but for consistency with Google API calls
    ...
    async def _extract_channel_company_domains(self, snapshots: List[VideoSnapshot]):
        """Extract company domains from YouTube channels using AI"""
        if not snapshots or not self.settings.OPENAI_API_KEY:
            return
        
        # Group snapshots by channel
        channels = {}
        for snapshot in snapshots:
            if snapshot.channel_id not in channels:
                channels[snapshot.channel_id] = {
                    'channel_id': snapshot.channel_id,
                    'channel_title': snapshot.channel_title,
                    'subscriber_count': snapshot.subscriber_count,
                    'description': getattr(snapshot, 'channel_description', ''),
                    'custom_url': getattr(snapshot, 'channel_custom_url', ''),
                    'snapshots': []
                }
            channels[snapshot.channel_id]['snapshots'].append(snapshot)
        
        # Extract domains from each channel
        extracted_domains = []
        for channel_id, channel_data in channels.items():
            try:
                result = await self._extract_domain_from_channel(channel_data)
                
                if result['domain']:
                    # Store the extracted domain
                    await self._store_channel_domain(
                        channel_id,
                        result
                    )
                    
                    # Add to list for company enrichment
                    ...
                    # Update snapshots with domain info
                    for snapshot in channel_data['snapshots']:
                        snapshot.channel_company_domain = result['domain']
                        snapshot.channel_source_type = result['source_type']
                    
            except Exception as e:
                logger.error(f"Error extracting domain from channel {channel_id}: {e}")
        
        # Return extracted domains for potential company enrichment
        if extracted_domains:
            logger.info(f"Extracted {len(extracted_domains)} company domains from YouTube channels")
            await self._queue_domains_for_enrichment(extracted_domains)
    
    async def _extract_domain_from_channel(
        self, 
        channel_data: Dict
    ) -> Dict[str, Any]:
        """Extract company domain from YouTube channel using AI"""
        try:
            from openai import OpenAI
            client = OpenAI(api_key=self.settings.OPENAI_API_KEY)
            
            # First, try to extract domain from description using regex
            ...
            channel_context = f"""
            Channel Title: {channel_data['channel_title']}
            Subscribers: {channel_data['subscriber_count']:,}
            Channel Description: {description[:1000]}
            Custom URL: {channel_data['custom_url']}
            URLs found in description: {', '.join(matches) if matches else 'None'}
            """
            
            # Define extraction function
            function_schema = {
                "name": "extract_company_domain",
                "description": "Extract company domain and classify YouTube channel",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "domain": {
                            "type": "string",
                            "description": "The primary company domain (e.g., company.com)"
                        },
                        "company_name": {
                            "type": "string",
                            "description": "The company name that owns this channel"
                        },
                        "source_type": {
                            "type": "string",
                            "enum": ["OWNED", "COMPETITOR", "INFLUENCER", "MEDIA", "EDUCATIONAL", "AGENCY", "OTHER"],
                            "description": "Type of channel owner"
                        },
                        "confidence": {
                            "type": "number",
                            "minimum": 0,
                            "maximum": 1,
                            "description": "Confidence score for the domain extraction (0-1)"
                        }
                    },
                    "required": ["domain", "company_name", "source_type", "confidence"]
                }
            }
            
            # Create the prompt
            prompt = f"""
            Analyze the following YouTube channel and extract the company domain that owns or operates this channel.
            
            {channel_context}
            
            Instructions:
            1. Look for company websites mentioned in the channel description
            2. Analyze the channel title for company names
            3. Check if the custom URL contains a company identifier
            4. If multiple domains are found, identify the primary company domain
            5. Assess confidence based on clarity of ownership signals
            
            Domain Extraction Guidelines:
            - Extract the main company domain (not social media links)
            - Remove www. prefix
            - Use the root domain (e.g., company.com, not subdomain.company.com)
            - If no clear domain is found, set domain to empty string
            
            Source Type Guidelines:
            - OWNED: Official company channel
            - COMPETITOR: Known competitor channel
            - INFLUENCER: Individual content creator
            - MEDIA: News outlet or media publication
            - EDUCATIONAL: Educational institution or training provider
            - AGENCY: Marketing or creative agency
            - OTHER: Doesn't fit the above categories
            
            Confidence Guidelines:
            - 0.9-1.0: Domain explicitly stated, official channel
            - 0.7-0.9: Strong indicators (company name matches, professional channel)
            - 0.5-0.7: Moderate indicators (some company references)
            - 0.0-0.5: Weak or no indicators
            """
            
            # Using the standard chat completions API with function calling
            # gpt-5-nano is perfect for this focused task - fast and accurate for domain extraction
            response = client.chat.completions.create(
                model="gpt-5-nano-2025-08-07",  # Ultra-fast nano model optimized for company/domain extraction
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert at identifying company ownership of YouTube channels and extracting company domains."
                    },
                    {
                        "role": "user",
                        "content": prompt
                ...
                tool_choice={"type": "function", "function": {"name": "extract_company_domain"}},
                # gpt-5-nano uses default temperature only
                max_completion_tokens=256    # Small token limit - we only need domain and company name
            )
            
            # Extract result from the chat completions API format
            ...
            logger.error(f"AI domain extraction error: {e}")
            return {
                'domain': '',
                'company_name': '',
                'source_type': 'OTHER',
    ...
    async def _get_client_info(self) -> Dict:
        """Get client company information"""
        try:
            async with self.db.acquire() as conn:
                result = await conn.fetchrow(
                    """
                    SELECT name, industry, domains
    ...
    async def _get_competitor_info(self) -> List[Dict]:
    ...
    async def _store_channel_domain(self, channel_id: str, result: Dict):
        """Store extracted channel domain in database"""
        try:
            async with self.db.acquire() as conn:
                await conn.execute(
                    """
            ...
            logger.error(f"Error storing channel domain: {e}")
    
    async def _queue_domains_for_enrichment(self, domains: List[Dict]):
        """Queue extracted domains for company enrichment"""
        try:
            async with self.db.acquire() as conn:
                # Insert domains into enrichment queue
                for domain_info in domains:
                    await conn.execute(
                        """
                        INSERT INTO company_enrichment_queue (
                            domain, source_type, source_id, confidence_score,
                            priority, created_at
                        ) VALUES ($1, $2, $3, $4, $5, NOW())
                        ON CONFLICT (domain) DO UPDATE SET
                            confidence_score = GREATEST(
                                company_enrichment_queue.confidence_score, 
                        ...
                        domain_info['domain'],
                        'youtube_channel',
                        domain_info['channel_id'],
                        domain_info['confidence'],
                        'medium'  # Priority level
                    )
                logger.info(f"Queued {len(domains)} domains for enrichment")
        except Exception as e:
            logger.error(f"Error queueing domains: {e}") 