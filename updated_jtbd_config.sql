-- ================================
-- UPDATED JTBD PHASES CONFIGURATION
-- Based on sophisticated buyer journey with detailed scoring criteria
-- ================================
...
UPDATE analysis_config SET jtbd_phases = '[
  {
    ...
    "scoring_framework": {
      "levels": [
        {"range": [0, 0], "description": "No mention of challenges/problems"},
        {"range": [1, 3], "description": "Vague references (businesses face challenges) without specificity"},
        {"range": [4, 6], "description": "Defines problem with some context (e.g., Healthcare organizations struggle with legacy EMR systems)"},
        {"range": [7, 8], "description": "Uses evidence (data, industry insight, consequences of inaction)"},
        {"range": [9, 10], "description": "Clear, specific, contextualized problem framing, quantified stakes, industry examples"}
      ],
  ...
  },
  {
    ...
    "scoring_framework": {
      "levels": [
        {"range": [0, 0], "description": "No solutions discussed"},
        {"range": [1, 3], "description": "Mentions a product vaguely without context"},
        {"range": [4, 6], "description": "Identifies general solution categories (e.g., cloud migration vs. on-prem)"},
        {"range": [7, 8], "description": "Provides pros/cons, trade-offs between approaches"},
        {"range": [9, 10], "description": "Rich comparative analysis, contextualized for industry/role"}
      ],
      "evidence_requirements": {
        "min_words": 120,
      ...
      "contextual_rules": [
        ...
        {"condition": "single_vendor_focus", "adjustment": -1, "description": "Deduct if only vendor's product mentioned as the solution"}
      ]
    }
  },
  {
    ...
    "scoring_framework": {
      "levels": [
        {"range": [0, 0], "description": "No requirements"},
        {"range": [1, 3], "description": "Vague (solution should be secure and easy to use)"},
        {"range": [4, 6], "description": "Several requirements defined, partial detail"},
        {"range": [7, 8], "description": "Detailed criteria with measurable or role-specific indicators"},
        {"range": [9, 10], "description": "Robust requirements framework, industry benchmarks, scoring matrices, or checklists"}
      ],
      "evidence_requirements": {
        "min_words": 150,
  ...
  },
  {
    ...
    "ai_context": {
      ...
      "negative_signals": ["We're the best style content without evidence", "Promotional content without substance"],
      "exclusions": ["Lists of vendors without context"]
    },
    "scoring_framework": {
      "levels": [
        {"range": [0, 0], "description": "No vendor evaluation context"},
        {"range": [1, 3], "description": "Mentions single supplier without comparison"},
        {"range": [4, 6], "description": "Lists some selection factors but generic"},
        {"range": [7, 8], "description": "Uses case studies, evaluation rubrics, industry references"},
        {"range": [9, 10], "description": "Structured comparison, includes validation sources (analyst reports, third-party benchmarks)"}
      ],
      "evidence_requirements": {
        "min_words": 120,
  ...
  },
  {
    ...
    "scoring_framework": {
      "levels": [
        {"range": [0, 0], "description": "No validation"},
        {"range": [1, 3], "description": "Weak, generic claims (trusted by leading companies)"},
        {"range": [4, 6], "description": "At least one case study or testimonial"},
        {"range": [7, 8], "description": "Multiple forms of validation, with quantifiable outcomes"},
        {"range": [9, 10], "description": "Rich validation (certifications, customer results, analyst references, ROI models)"}
      ],
      "evidence_requirements": {
        "min_words": 100,
...
-- Verify the update
SELECT 
    'JTBD PHASES UPDATE COMPLETE' as status,
